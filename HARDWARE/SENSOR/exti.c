...
void EXTIX_Init(void)
{
  ...
  RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOB|RCC_AHB1Periph_GPIOF|RCC_AHB1Periph_GPIOC|RCC_AHB1Periph_GPIOA, ENABLE);//使能GPIOB,GPIOG,GPIOC,GPIOA时钟
  ...
  GPIO_Init(GPIOB, &GPIO_InitStructure);//初始化GPIOB0
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_11; //PG1对应IO配置
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN;//普通输入模式
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;//100M
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;//上拉
  GPIO_Init(GPIOF, &GPIO_InitStructure);//初始化GPIOG1
  ...
  GPIO_Init(GPIOA, &GPIO_InitStructure);//初始化GPIOA3
	...
	SYSCFG_EXTILineConfig(EXTI_PortSourceGPIOF, EXTI_PinSource11);//PF11 连接到中断线1
	...
	//配置中断线
  EXTI_InitStructure.EXTI_Line = EXTI_Line0|EXTI_Line11|EXTI_Line2|EXTI_Line3;//LINE0
